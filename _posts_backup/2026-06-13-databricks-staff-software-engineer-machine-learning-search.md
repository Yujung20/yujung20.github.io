---
layout: post
title: "databricks - Staff Software Engineer - Machine Learning (Search)"
date: 2026-06-13
company: "databricks"
role: "Staff Software Engineer - Machine Learning (Search)"
...
`Machine Learning` `Search Ranking` `NLP` `Information Retrieval` `ML Pipelines` `Query Understanding` `Relevance Models` `Applied AI` `Data Engineering` `Evaluation Frameworks`

---

## 공고 요약

Databricks의 Applied AI 팀에서 검색 품질 향상을 이끌 Staff Software Engineer를 채용합니다. 수억 개의 데이터 자산(테이블, 노트북, 대시보드 등)에 대한 검색 및 발견 기능을 ML 기반으로 강화하는 역할입니다. 2025년에는 검색 랭킹 개선, 쿼리 이해, 평가 시스템 구축, 자산 커버리지 확대에 집중할 예정입니다. ML/NLP 파이프라인 설계 및 배포를 주도하며 Databricks 플랫폼과 통합된 검색 관련성 모델을 개발하게 됩니다.

---

## 필수 지원 조건

- ML 기반 검색 랭킹 및 관련성 모델 개발 및 배포 경험
- NLP 파이프라인 설계 및 쿼리 이해(Query Understanding) 시스템 구현 능력
- 대규모 데이터 환경에서의 ML 시스템 설계 및 운영 경험
- 검색/정보 검색(Information Retrieval) 분야의 실무 경험
- Staff 레벨에 걸맞는 기술 리더십 및 팀 주도 경험
...
- NLP 및 쿼리 이해 모델을 실제 프로덕션에 적용해 본 경험자
- 대규모 플랫폼 환경에서 검색 품질 개선 프로젝트를 리드해 본 시니어 엔지니어
- Databricks, Spark 등 데이터 플랫폼 생태계에 친숙한 개발자
- ML 평가(Eval) 프레임워크 구축 및 모델 성능 측정에 관심 있는 엔지니어
