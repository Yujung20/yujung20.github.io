---
layout: post
title: "databricks - Engineering Manager - Lakebase Storage"
date: 2026-06-15
company: "databricks"
role: "Engineering Manager - Lakebase Storage"
...
`Engineering Manager` `Lakebase` `PostgreSQL` `Distributed Systems` `Rust` `Storage Technology` `Data Platform` `Lakehouse` `AI Infrastructure` `Databricks`

---

## 공고 요약

Databricks는 런던 오피스 확장을 위해 Lakebase Storage 팀의 창립 엔지니어링 리더를 채용합니다. Lakebase는 관리형 Postgres 트랜잭션 레이어를 제공하며, 운영 데이터, 분석, AI를 결합한 애플리케이션 워크로드를 지원합니다. 스토리지 기술, 분산 시스템, Rust 전문가 팀을 이끌며 미션 크리티컬한 서비스를 구축하는 역할을 담당합니다. 런던 조직의 초기 멤버로서 제품과 팀 문화를 함께 만들어가는 기회입니다.

---

## 필수 지원 조건

- 스토리지 기술 및 분산 시스템 분야의 엔지니어링 팀 관리 경험
- Rust 기반 시스템 개발 팀 리딩 경험
- 미션 크리티컬한 대규모 데이터 플랫폼 서비스 개발 및 운영 경험
- 지속적 배포(Continuous Delivery) 프로세스 운영 역량

---

## 추천 대상

- 분산 시스템 또는 데이터베이스 인프라 분야에서 엔지니어링 매니저 경험이 있는 분
- Postgres, 스토리지 엔진 등 데이터베이스 내부 기술에 깊은 이해를 가진 기술 리더
- 스타트업처럼 조직과 제품을 초기부터 함께 만들어가는 경험을 원하는 분
- Rust 또는 시스템 프로그래밍 백그라운드를 가진 엔지니어링 리더
