---
layout: post
title: "mongodb - Senior Software Engineer, Replicated Storage Services"
date: 2026-06-17
company: "mongodb"
role: "Senior Software Engineer, Replicated Storage Services"
location: "Toronto"
source: "greenhouse"
...
`Distributed Systems` `Cloud-Native` `Replicated Storage` `Database Engineering` `Storage Architecture` `Replication Platform` `API Design` `MongoDB` `Backend Engineering` `System Design`

---

## 공고 요약

MongoDB의 차세대 클라우드 스토리지 및 복제 플랫폼을 구축하는 Next-Gen Platform Team에서 시니어 소프트웨어 엔지니어를 채용합니다. 스토리지 레이어와 아키텍처를 클라우드 네이티브 방식으로 현대화하는 작업을 담당합니다. API 및 인터페이스 경계 개선을 통해 엔터프라이즈급 성능과 정확성을 제공하는 것이 주요 목표입니다. 미국 및 캐나다 기반으로 원격 근무가 가능하며, 최소 5년 이상의 경력을 요구합니다.

---

## 필수 지원 조건

- 최소 5년 이상의 소프트웨어 엔지니어링 경력
- 분산 시스템(Distributed Systems) 관련 실무 경험
- 클라우드 네이티브 스토리지 또는 데이터베이스 시스템 설계 및 개발 경험
- API 설계 및 인터페이스 경계 정의 능력
- 미국 또는 캐나다 거주 가능자
...
- 분산 데이터베이스 또는 스토리지 시스템 개발 경험이 있는 엔지니어
- 클라우드 네이티브 아키텍처 설계에 관심 있는 백엔드 개발자
- 대규모 엔터프라이즈 환경에서의 성능 최적화 경험자
- 협업과 다양성을 중시하는 팀 문화를 선호하는 개발자
- 데이터베이스 내부 구조와 복제 메커니즘에 깊은 관심이 있는 시스템 엔지니어
