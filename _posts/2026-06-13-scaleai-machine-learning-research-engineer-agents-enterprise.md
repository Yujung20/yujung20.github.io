---
layout: post
title: "scaleai - Machine Learning Research Engineer, Agents - Enterprise GenAI"
date: 2026-06-13
company: "scaleai"
role: "Machine Learning Research Engineer, Agents - Enterprise GenAI"
...
`Machine Learning` `Reinforcement Learning` `Agents` `GenAI` `Post-training` `LLM` `Enterprise AI` `Research Engineering` `AI Data` `Generative AI`

---

## 공고 요약

Scale AI는 엔터프라이즈 환경에서 동작하는 AI 에이전트 개발을 위한 머신러닝 리서치 엔지니어를 채용합니다. 주요 업무는 Agent RL Training 및 알고리즘을 실제 엔터프라이즈 데이터셋에 적용하고, 최신 성능의 에이전트를 구축하는 것입니다. Meta의 투자를 바탕으로 최첨단 포스트 트레이닝 알고리즘 개발에 집중하고 있으며, 다양한 글로벌 엔터프라이즈 클라이언트를 위한 AI 솔루션을 연구합니다. San Francisco 또는 New York에서 근무하며 AI 혁신의 최전선에서 활동하게 됩니다.

---

## 필수 지원 조건

- 강화학습(RL) 및 에이전트 학습 알고리즘에 대한 깊은 이해와 실무 경험
- 대규모 언어 모델(LLM) 및 생성형 AI 관련 연구 또는 개발 경험
- 엔터프라이즈 데이터셋을 활용한 머신러닝 모델 학습 및 평가 경험
- 최신 AI/ML 연구 논문을 이해하고 실제 시스템에 구현할 수 있는 능력

---

## 추천 대상

- 강화학습 기반 AI 에이전트 연구 및 개발 경험이 있는 ML 엔지니어
- LLM 포스트 트레이닝(RLHF, PPO 등) 분야에 관심 있는 연구자
- 학계 또는 산업계에서 GenAI 관련 프로젝트를 수행한 경험자
- 실제 비즈니스 문제에 최신 AI 기술을 적용하는 데 열정이 있는 개발자
- 빠르게 성장하는 AI 스타트업 환경에서 연구와 엔지니어링을 병행하고 싶은 사람
