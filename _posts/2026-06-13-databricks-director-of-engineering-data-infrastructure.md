---
layout: post
title: "databricks - Director of Engineering (Data Infrastructure)"
date: 2026-06-13
company: "databricks"
role: "Director of Engineering (Data Infrastructure)"
...
`Data Infrastructure` `Engineering Director` `Distributed Systems` `Disaster Recovery` `Billing Systems` `Site Reliability` `Large-scale Data Processing` `Team Leadership` `Cloud Infrastructure`

---

## 공고 요약

Databricks는 인도 벵갈루루에서 데이터 인프라 조직을 이끌 Engineering Director를 찾고 있습니다. 이 역할은 하루 수십억 건의 트랜잭션과 페타바이트급 데이터를 처리하는 핵심 인프라를 책임집니다. 99.999% 정확도의 빌링 시스템, 재해 복구 체계, 대규모 테스트 프레임워크 구축을 주도해야 합니다. 벵갈루루에 기반 팀을 새롭게 설립하고 조직의 성장을 견인할 리더십이 핵심입니다.

---

## 필수 지원 조건

- 대규모 분산 데이터 인프라 시스템 설계 및 운영 경험
- 엔지니어링 팀 빌딩 및 조직 리더십 경험 (Director급 이상)
- 고가용성(99.999%) 및 재해 복구 시스템 구축 경험
- 빌링/트랜잭션 시스템 등 미션 크리티컬 인프라 운영 경험
- 멀티 리전 클라우드 인프라 운영 및 확장 경험

---

## 추천 대상

- 글로벌 빅테크 또는 클라우드 기업에서 대규모 인프라 팀을 이끈 시니어 엔지니어링 리더
- 데이터 파이프라인, 스트리밍 인프라, 분산 시스템 전문가로 조직 신설 경험이 있는 분
- 인도 벵갈루루에서 새로운 엔지니어링 허브를 구축하는 데 관심 있는 리더
- 빌링·트랜잭션 정확성 및 신뢰성 문제를 기술적으로 해결한 경험이 있는 분
- 0→1 팀 빌딩과 하이퍼스케일 운영 모두 경험한 풀사이클 엔지니어링 리더
