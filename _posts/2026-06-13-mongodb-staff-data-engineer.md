---
layout: post
title: "mongodb - Staff Data Engineer"
date: 2026-06-13
company: "mongodb"
...
`ETL Pipelines` `Apache Spark` `Big Data` `Data Engineering` `Medallion Architecture` `Cloud Infrastructure` `AI Agents` `Internal Data Platform` `Data Governance` `Python`

---

## 공고 요약

MongoDB의 Data Engineering 팀은 내부 데이터 플랫폼을 구축하는 ETL 파이프라인 개발을 담당합니다. Staff Data Engineer로서 Spark 등 빅데이터 기술을 활용한 고성능 ETL 파이프라인 설계 및 Medallion 아키텍처 구현을 리드해야 합니다. 클라우드 비용 최적화 전략을 제시하고, AI 에이전트를 설계하여 개발 및 운영 업무를 자동화하는 역할도 포함됩니다. 구르가온 기반의 하이브리드 근무 방식으로 운영되며, 보안 및 컴플라이언스 팀과 협업하여 데이터 권한 관리도 수행합니다.

---

## 필수 지원 조건

- Apache Spark를 활용한 대규모 고성능 ETL 파이프라인 설계 및 구축 경험
- Medallion Architecture 등 데이터 플랫폼 아키텍처 설계 역량
- AWS, GCP, Azure 등 클라우드 하이퍼스케일러 인프라 비용 최적화 경험
- AI 에이전트 설계 및 개발 능력
- 인도 구르가온 기반 하이브리드 근무 가능자

---

## 추천 대상

- 빅데이터 ETL 파이프라인 개발 및 아키텍처 설계에 풍부한 경험을 가진 시니어 엔지니어
- Spark, 데이터 레이크하우스 등 대규모 데이터 처리 플랫폼 구축 경험자
- 클라우드 인프라 비용 최적화 및 데이터 거버넌스에 관심 있는 엔지니어
- AI/LLM 기반 자동화 에이전트 개발에 관심 있는 데이터 엔지니어
- 기술 리더십을 발휘하며 팀을 가이드하고 싶은 Staff/Principal급 엔지니어
