---
layout: post
title: "mongodb - Principal Research Scientist, Database Systems"
date: 2026-06-15
company: "mongodb"
role: "Principal Research Scientist, Database Systems"
...
`Database Systems` `Query Optimization` `Storage Engines` `Indexing` `Multicore Processing` `Database Architecture` `Performance Optimization` `Research Scientist` `Scalability` `MongoDB`

---

## 공고 요약

MongoDB 리서치 팀은 데이터베이스 시스템 분야의 기초 및 응용 연구를 통해 제품 혁신을 주도할 수석 연구 과학자를 찾고 있습니다. 쿼리 최적화, 스토리지 엔진, 인덱싱, 멀티코어 처리 등 데이터베이스 아키텍처 전반에 걸친 깊은 전문성이 요구됩니다. 고성능 데이터베이스 시스템 설계 및 구현 경험이 있으며, 학계 및 연구 기관과의 협업 및 논문 발표 실적이 있는 인재를 우대합니다. 북미 지역 원격 근무도 가능한 유연한 근무 형태를 제공합니다.

---

## 필수 지원 조건

- 데이터베이스 시스템 분야 전문 연구 경력 및 강력한 논문 발표 실적
- 쿼리 최적화, 스토리지 엔진, 인덱싱, 멀티코어 처리 등 데이터베이스 아키텍처 전반에 대한 깊은 전문 지식
- 고성능 데이터베이스 시스템 설계 및 실제 구현 경험
- 데이터 관리, 성능 최적화, 확장성, 보안 등 핵심 기술 문제 해결 능력

---

## 추천 대상

- 데이터베이스 시스템 분야 박사 학위 보유 및 연구 경험이 풍부한 연구자
- 학술 논문 발표 및 선도적 연구 기관과의 협업 경험이 있는 전문가
- 쿼리 엔진, 스토리지 최적화 등 DB 내부 구조에 관심이 많은 시스템 연구자
- 산업계와 학계를 연결하는 응용 연구에 관심 있는 컴퓨터 과학 연구자
