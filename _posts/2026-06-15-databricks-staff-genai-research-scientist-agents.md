---
layout: post
title: "databricks - Staff GenAI Research Scientist - Agents"
date: 2026-06-15
company: "databricks"
role: "Staff GenAI Research Scientist - Agents"
...
`GenAI` `Agents` `RAG` `LLM` `Embeddings` `Information Retrieval` `Multi-Agent Architecture` `Knowledge Assistant` `Post-training` `AI Research`

---

## 공고 요약

Databricks의 Enterprise Agents 팀에서 Staff Research Scientist를 채용합니다. 이 역할은 에이전트 컨텍스트, 정보 검색, RAG, 임베딩 등 최신 AI 에이전트 기술 연구를 담당합니다. 오픈소스 LLM 후처리부터 멀티에이전트 아키텍처 개발까지 폭넓은 AI 연구를 수행하며, 기업이 자체 데이터로 고품질 AI 모델과 에이전트를 구축할 수 있도록 지원합니다. 뉴욕 오피스 기반으로 세계 최고 수준의 데이터·AI 플랫폼을 함께 만들어갈 연구자를 찾고 있습니다.

---

## 필수 지원 조건

- LLM, RAG, 임베딩, 정보 검색 등 생성형 AI 및 에이전트 분야의 깊은 연구 경험
- 멀티에이전트 아키텍처 설계 및 구현 능력
- 최신 AI/ML 논문을 빠르게 파악하고 실제 시스템에 적용할 수 있는 역량
- Staff 레벨에 준하는 시니어급 연구 경력 및 기술 리더십

---

## 추천 대상

- RAG, 에이전트, LLM 후처리(post-training) 연구 경험이 풍부한 AI 연구자
- 기업용 Knowledge Assistant 또는 대화형 AI 시스템을 개발해본 경험자
- 오픈소스 LLM 생태계에 기여하거나 관련 논문을 발표한 경험이 있는 분
- 대규모 데이터 플랫폼 환경에서 AI 연구를 실용화한 경험이 있는 분
- 멀티에이전트 시스템 또는 복잡한 AI 파이프라인 아키텍처에 관심 있는 시니어 연구자
