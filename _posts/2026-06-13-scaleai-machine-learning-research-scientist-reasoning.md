---
layout: post
title: "scaleai - Machine Learning Research Scientist, Reasoning"
date: 2026-06-13
company: "scaleai"
role: "Machine Learning Research Scientist, Reasoning"
...
`Machine Learning` `LLM` `Reasoning` `AI Research` `NLP` `Generative AI` `LLM Agents` `Data Strategy` `AGI` `Software Engineering Agents`

---

## 공고 요약

Scale AI는 대형 언어 모델(LLM)의 추론 능력 연구에 집중할 머신러닝 리서치 사이언티스트를 채용합니다. 이 역할은 LLM 기반 에이전트(브라우저 및 소프트웨어 엔지니어링 에이전트 포함)의 발전을 위한 핵심 데이터 유형을 연구하고, Scale의 데이터 전략 수립에 주도적으로 참여합니다. AI 연구와 실제 구현 최전선에서 활동하며, AGI 발전을 위한 고품질 데이터 확보 및 평가 기준 정립에 기여하게 됩니다.

---

## 필수 지원 조건

- 대형 언어 모델(LLM) 및 추론(Reasoning) 관련 연구 경험
- AI/ML 분야 연구 역량 (논문 발표 또는 관련 프로젝트 경험)
- LLM 기반 에이전트 또는 생성형 AI 시스템 개발·분석 경험
- 데이터 전략 수립 및 고품질 학습 데이터 설계 능력

---

## 추천 대상

- LLM 추론 및 에이전트 연구에 깊은 관심을 가진 AI 연구자
- 생성형 AI 또는 NLP 분야에서 연구 경험을 보유한 머신러닝 엔지니어
- AGI 발전에 기여하고 싶은 데이터 중심의 AI 과학자
- 학계 또는 산업계에서 AI 모델 평가 및 데이터 전략 경험이 있는 전문가
