---
layout: post
title: "block - Senior Machine Learning Engineer, Model Risk Management"
date: 2026-06-13
company: "block"
role: "Senior Machine Learning Engineer, Model Risk Management"
...
`Machine Learning` `Model Risk Management` `Financial Services` `Credit Risk` `Fair Lending` `AML` `Model Validation` `Python` `Statistical Modeling` `FinTech`

---

## 공고 요약

Block(Square, Cash App 등의 모회사)은 금융 서비스 전반에 걸쳐 대규모로 운영되는 ML 모델의 리스크를 독립적으로 검증하고 관리할 시니어 ML 엔지니어를 찾고 있습니다. 이 역할은 신용 손실, 금융 범죄 탐지, 공정 대출 규정 준수 등 비즈니스에 직접적인 영향을 미치는 모델의 건전성을 평가하는 독립 기능을 담당합니다. 뉴욕에 위치하며, 모델 검증 및 리스크 관리 경험을 갖춘 전문가를 필요로 합니다.

---

## 필수 지원 조건

- 머신러닝 모델 검증 및 리스크 관리 실무 경험
- 신용 리스크, AML(자금세탁방지), 공정 대출 관련 금융 규제 모델에 대한 이해
- Python 등을 활용한 통계적 모델링 및 정량 분석 능력
- 독립적 모델 검증 프로세스 수행 및 문서화 역량
- 대규모 금융 서비스 환경에서의 ML 시스템 운영 경험

---

## 추천 대상

- 금융권 모델 리스크 관리(MRM) 또는 모델 검증 경험이 있는 ML 엔지니어
- 신용 평가, 사기 탐지, 금융 범죄 관련 ML 모델 개발 경험자
- 규제 환경(OCC, Fed 가이드라인 등)에서 모델 거버넌스 업무를 해본 분
- 독립적이고 비판적인 시각으로 타 팀 모델을 검토하는 것에 흥미 있는 분
- FinTech 또는 금융 서비스 산업에서 임팩트 있는 ML 업무를 원하는 시니어급 엔지니어
