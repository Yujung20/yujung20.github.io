---
layout: post
title: "cockroachlabs - Member of Technical Staff (Storage)"
date: 2026-06-15
company: "cockroachlabs"
role: "Member of Technical Staff (Storage)"
...
`Storage Engine` `LSM Tree` `MVCC` `Pebble` `CockroachDB` `Distributed Database` `Go` `Systems Programming` `Database Internals`

---

## 공고 요약

CockroachDB의 Storage 팀에서 스토리지 엔진 개발을 담당할 소프트웨어 엔지니어를 채용합니다. 해당 팀은 CockroachDB의 자체 스토리지 엔진인 Pebble을 개발 및 유지보수하며, 데이터의 안전하고 내구성 있는 영속성과 검색을 책임집니다. Pebble은 MVCC 데이터를 위해 특별히 설계된 혁신적인 LSM Tree 기반 스토리지 엔진입니다. 데이터베이스의 가장 핵심 레이어를 다루는 팀으로, 고성능 분산 데이터베이스의 근간을 설계하고 개선하는 역할을 수행합니다.

---

## 필수 지원 조건

- LSM Tree, B-Tree 등 스토리지 엔진 내부 구조에 대한 깊은 이해
- MVCC(Multi Version Concurrency Control) 개념 및 구현 경험
- Go 또는 C++ 등 시스템 프로그래밍 언어 숙련도
- 데이터베이스 내부 구조(스토리지, I/O, 캐싱 등)에 대한 전문 지식
- 분산 시스템 또는 고성능 데이터 영속성 시스템 개발 경험

---

## 추천 대상

- 데이터베이스 스토리지 엔진 개발에 깊은 관심이 있는 시스템 엔지니어
- RocksDB, LevelDB, Pebble 등 LSM 기반 스토리지 엔진 경험자
- 로우레벨 성능 최적화 및 디스크 I/O 튜닝에 흥미를 가진 개발자
- 오픈소스 데이터베이스 프로젝트에 기여한 경험이 있는 엔지니어
- 뉴욕에서 근무 가능하며 최첨단 분산 데이터베이스 핵심 기술을 개발하고 싶은 개발자
